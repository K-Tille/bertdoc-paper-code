# Model Testing Guide for the models series of the paper: "Evaluation of transformer-based relevance models for clinical information retrieval"
...
1. start => docker run --gpus all -p 7777:7777 bertdoc_colbert:latest
2. Look for a token for jupyter e.g. =>  http://127.0.0.1:7777/tree?token=2df1de0b1668d6811b09
3. run _complete_colbert_example.ipynb

## Requirement for Sentence Transformer (no GPU)

The Sentence Transformers dont need a GPU.

1. Start => docker run -p 5555:5555 bertdoc_sentence:latest
2. Look for a token for jupyter e.g. =>  http://127.0.0.1:7777/tree?token=2df1de0b1668d6811b09
3. run _complete_sentence_transformer_example.ipynb


## Results in Notebooks

The evaluation results for each model are stored in their respective Jupyter notebooks:

_complete_colbert_example.ipynb

_complete_sentence_transformer_example.ipynb

Each notebook contains the workflow for training, evaluation and statistics for the respective model series.
